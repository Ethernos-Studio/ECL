...
export function activate(context: vscode.ExtensionContext) {
    ...
    // 注册代码补全提供者
    const eclCompletionProvider = vscode.languages.registerCompletionItemProvider(
        'ecl',
        new ECLCompletionProvider(),
        '.', // 在输入点时触发
        ' ', // 在输入空格时触发
        '('  // 在输入括号时触发
    );
    ...
    // 注册悬停提示提供者
    const eclHoverProvider = vscode.languages.registerHoverProvider(
        'ecl',
        new ECLHoverProvider()
    );

    // 注册文档符号提供者
    const eclDocumentSymbolProvider = vscode.languages.registerDocumentSymbolProvider(
        'ecl',
        new ECLDocumentSymbolProvider()
    );

    // 注册定义提供者（支持Ctrl+点击跳转）
    const eclDefinitionProvider = vscode.languages.registerDefinitionProvider(
        'ecl',
        new ECLDefinitionProvider()
    );

    // 注册引用提供者（支持查找所有引用）
    const eclReferenceProvider = vscode.languages.registerReferenceProvider(
        'ecl',
        new ECLReferenceProvider()
    );

    context.subscriptions.push(
        eclCompletionProvider,
        eclHoverProvider,
        eclDocumentSymbolProvider,
        eclDefinitionProvider,
        eclReferenceProvider
    );
}
...
// 代码补全提供者
class ECLCompletionProvider implements vscode.CompletionItemProvider {
...
// 悬停提示提供者
class ECLHoverProvider implements vscode.HoverProvider {
    provideHover(
        document: vscode.TextDocument,
        position: vscode.Position,
        token: vscode.CancellationToken
...
// 文档符号提供者
class ECLDocumentSymbolProvider implements vscode.DocumentSymbolProvider {
...
class ECLDefinitionProvider implements vscode.DefinitionProvider {
    provideDefinition(
        document: vscode.TextDocument,
        position: vscode.Position,
        token: vscode.CancellationToken
    ...
    private findDefinitionInDocument(document: vscode.TextDocument, word: string): vscode.Location | undefined {
        const text = document.getText();
        const lines = text.split('\n');
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (funcMatch) {
                ...
                return new vscode.Location(
                    document.uri,
                    new vscode.Range(i, funcNameIndex, i, funcNameIndex + word.length)
                );
            }
            ...
            if (varMatch) {
                ...
                return new vscode.Location(
                    document.uri,
                    new vscode.Range(i, varNameIndex, i, varNameIndex + word.length)
                );
            }
        }
    ...
    private findImportedSymbols(document: vscode.TextDocument, word: string): Thenable<vscode.Location | undefined> {
        const text = document.getText();
        const lines = text.split('\n');
        ...
        const searchPromises = importStatements.map(importFile => {
            ...
            // 返回第一个找到的定义
            return Promise.all(pathPromises).then((results: (vscode.Location | undefined)[]) => {
                return results.find((result: vscode.Location | undefined) => result !== undefined);
            });
        });

        // 返回所有import语句的查找结果中的第一个有效定义
        return Promise.all(searchPromises).then((allResults: (vscode.Location | undefined)[]) => {
            return allResults.find((result: vscode.Location | undefined) => result !== undefined);
        });
    }

    private extractImportStatements(lines: string[]): string[] {
...
// 引用提供者（支持查找所有引用）
class ECLReferenceProvider implements vscode.ReferenceProvider {
    provideReferences(
        document: vscode.TextDocument,
        position: vscode.Position,
